    ...
    <>
      ...
      <section className="mx-auto max-w-editorial px-6 py-12 md:py-16">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl text-ink">{registries.length} registr{registries.length === 1 ? "y" : "ies"}</h2>
        ...
        {open && (
          <form onSubmit={submit} className="mt-8 max-w-lg space-y-4 border border-ink/12 bg-white p-6 shadow-sm md:p-8">
            <div>
              ...
              <input className={inputCls} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Aira's 25th Birthday" />
            </div>
            <div>
            ...
            <div>
              <label className="mb-1.5 block text-[0.7rem] uppercase tracking-[0.14em] text-ink/55">Note to guests <span className="text-ink/35">(optional)</span></label>
              <input className={inputCls} value={note} onChange={(e) => setNote(e.target.value)} placeholder="A little message…" />
            </div>
        ...
        {registries.length > 0 && (
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {registries.map((r) => (
              <Link key={r.id} href={`/registry/${r.id}`} className="group border border-ink/12 bg-white p-6 transition-colors hover:border-wine">
                <p className="font-display text-xl text-ink">{r.title}</p>
                {r.event_date && <p className="mt-1 text-xs uppercase tracking-[0.14em] text-ink/45">{new Date(r.event_date).toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" })}</p>}
                <p className="mt-4 text-[0.7rem] uppercase tracking-[0.16em] text-wine">Manage &amp; share →</p>
              </Link>
            ))}
          </div>
        )}
